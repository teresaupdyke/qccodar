...
In[]: %run qcviz.py /Users/codar/Documents/qccodar_dev/tools/2017_01 IdealPattern RDLv_HATY_2017_01_01_0400.ruv
In[]: plt.show()
...
def stest0_change(val):
    ...
    params['thresholds'][0] = stest0.seqvals[val]
    update_configs()
...
def stest1_change(val):
    global params, d, types_str, rsd, rstypes_str
    params['thresholds'][1] = stest1.seqvals[val]
    update_configs()
...
def stest2_change(val):
    global params, d, types_str, rsd, rstypes_str
    params['thresholds'][2] = stest2.seqvals[val]
    update_configs()
...
sbear = Slider(axbear, 'Bearing', seqvals=list(range(0,269,1)), valinit=0, valfmt='%03d (deg)')
...
# def subset_rsdata(rsd, rsc, bearing):
    ...
    rsd_df = copy.deepcopy(rsx.data)
    rsd_np = rsd_df.to_numpy()
    cols = ['VELO', 'SPRC', 'BEAR']

    # rsd as NumPy array
    xrow = numpy.where((rsd_np['BEAR']==bearing) & (rsd_np['VFLG']==0))[0]
    # [df.columns.get_loc(c) for c in cols if c in df]
    xcol = numpy.array([rsd_df.columns.get_loc(c) for c in cols if c in rsd_df])
    a_np = rsd_np[numpy.ix_(xrow, xcol)]

    # as DataFrame
    xrow = (rsd_df['BEAR']==bearing) & (rsd_df['VFLG']==0)
    a_df = rsd.loc[xrow, cols]
...
def plot_data(rmqc, rsx):
    ...
    rs = subset_rsdata(rsx.data, params['bearing'])
    gd = subset_data_good(rmqc.data, params['bearing'], params['numdegrees'])
    bd = subset_data_bad(rmqc.data, params['bearing'], params['numdegrees'])
